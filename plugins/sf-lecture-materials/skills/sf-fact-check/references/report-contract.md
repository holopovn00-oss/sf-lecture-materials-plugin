...
Числовой расчет: `{id, kind:"calculation", expression, result, method, applicability, limitations}`. По умолчанию выражения из десятичных чисел, скобок, +, -, *, /, ** вычисляются точно как рациональные; целая степень ограничена модулем 1000, result содержит точное число или дробь. Для дробных степеней, sqrt, ln/log (натуральный логарифм) и exp добавь `numeric:{precision:50, absolute_tolerance:"1e-14", relative_tolerance:"0"}`. Decimal precision допускает 28–100 знаков; оба допуска задаются явно в пределах 0–1e-6. Сравнение использует max абсолютного допуска и относительного допуска, умноженного на модуль вычисленного значения. Допуск обосновывается требуемой точностью, а не подбирается под неверный ответ. AST ограничен 100 узлами, выражение 1000 символами; произвольный Python-код не исполняется. Это арифметическая модель, не эмуляция binary float.
...
Assessment: `{status, rationale, evidence_ids, proposal, decision_required}`. Новые отчеты явно задают boolean decision_required; для старых записей отсутствие поля означает proposal != null. Status:
...
`decision_required:true` отмечает только реальную ошибку или существенную оговорку, предъявляемую человеку. Для error/outdated true обязателен, как и proposal. Необязательные рекомендации хранятся с false: они не блокируют полный цикл и не применяются автоматически. Любой явно заданный recheck остается блокирующим.
...
Применение только согласованных поправок не требует нового исследования всего текста: сохрани исходный отчет, новую редакцию и адреса применения. Full-cycle handoff проверяет точное соответствие всех изменений одобренным заменам, неизменность соседнего текста и структуры. Пересекающиеся предложения сначала согласуй в одну адресную замену. Если изменены иные утверждения, условия или структура, зафиксируй новую входную редакцию и проведи проверку затронутого содержания; не переноси старую отметку complete на нее автоматически. FC-ID и решения текстового этапа сохраняют связь с исходниками.

## Команда и предел результата

`python scripts/verify_fact_check.py --report <абсолютный путь>`

На stdout — JSON, код 0 при `FACT_CHECK_RECORD_VALIDATED`, 1 при `BLOCKED`. При необходимости сохрани stdout в отдельный check.json. Проверка не пишет вход/отчёт и не подключается к сети. В результате отдельно видны unresolved findings, pending decisions, actionable findings (decision_required=true), retained findings, непроверенные утверждения, записанный охват и пересчитанные выражения. Unresolved findings не включает выполненную поправку или явное допущение без предложения; оставленная ошибка остаётся замечанием. Status counts всегда описывает исходный вход данного отчёта. Истина, независимость исследований и полнота инвентаризации остаются `NOT_AUTHENTICATED_BY_SCRIPT`, полномочия человека — `RECORDED_NOT_AUTHENTICATED`.
